...
--select * from Products where UnitPrice >=1
--SELECT SUM (UnitPrice) price
--from Products;
...
--inner join Orders o
--on o.OrderID = od.OrderID --bir join daha eklemek istersek böyle kullanırız.
SELECT products.ProductName, SUM (od.Quantity*products.UnitPrice) AS TotalPrice 
FROM [Order Details] AS od
inner join Products AS products 
ON od.ProductID=products.ProductID
inner join Orders AS orders ON orders.OrderID=od.OrderID
